...
function isLink(o) {
    if(!o || !o.nodeName) return 0;
    if(o.nodeName == 'A') return 1;
...
function showCrosshairs(x,y) {
    ...
    crosshairs.style.left = ""+Math.floor(x-crosshairs.width/2)+"px";
    crosshairs.style.top = ""+Math.floor(y-crosshairs.height/2)+"px";
    crosshairs.style.display='';
}
...
function doScroll() {
    if(!isMobile()) return;
    setTimeout(window.scrollTo,250,0,9);
}
...
function touchstart(e) {
    ...
    e.preventDefault();
    state.touch = getTouch(e.touches[0]);
    setTimeout(showCrosshairs,0,state.touch.x,state.touch.y);
    doScroll();
}

function touchToKey(touch) {
    touch = getTouch(touch);
    var stouch = state.touch;
    if(!touch || !stouch) return -1;
    var dx = touch.x-stouch.x; var dy = touch.y-stouch.y; 
    var rot = ((Math.atan2(dy,dx)*180.0/Math.PI)+360.0)%360.0;
    var dist = Math.sqrt(dy*dy+dx*dx);
    var tolerance = 15.0;
    var ang = Math.round((rot+45.0)%90.0);
    var quad = Math.floor((rot+45.0)/90.0)%4;
...
function touchmove(e) {
    if(!e || !e.touches || e.touches.length != 1 || !state.touch) return;
    if(isLink(e.target)) return;
    e.preventDefault();
    var ndownkey = touchToKey(e.touches[0]);
    if(ndownkey != downkey) {
	clearInterval(keyitrv);
	downkey = ndownkey;
	if(ndownkey >= 0) {
	    keyitrv = setInterval(step,750);
	}
    }
}
...
function windowRotated() {
    doScroll();
    if(!isMobile()) return;
    var full = document.getElementById('blockdude');
    var mini = document.getElementById('blockmini');
    if(!full || !mini) return;
...
function Interface(env,w,h,cw,ch) {
	...
	if(!env) return;
	this.env = env; this.env.addInterface(this);
    }
    ...
    this.left = function() { if(!this.pan) return; this.panning = true; this.pan.x -= 1; this.update(); }
    this.right = function() { if(!this.pan) return; this.panning = true; this.pan.x += 1; this.update(); }
    this.up = function() { if(!this.pan) return; this.panning = true; this.pan.y += 1; this.update(); }
    this.down = function() { if(!this.pan) return; this.panning = true; this.pan.y -= 1; this.update(); }
    this.center = function() { this.panning = false; this.update(); }
	...
	this.pan.x = Math.floor(this.pan.x); this.pan.y = Math.floor(this.pan.y);
	for(var y = 0; y < this.h; y++) {
	    for(var x = 0; x < this.w; x++) {
...
function Environment(w,h) {
	...
	var x = e.x||0; var y = e.y||0;
	if(!this.valid(x,y)) return null;
	this.grid[y][x] = e;
	return {'x': x, 'y': y};
    }
    this.moveElement = function(r,e) {
	var ox = r.x||0; var oy = r.y||0;
	var x = e.x||0; var y = e.y||0;
...
function Element() {
    ...
    this._fall = function() {
	while(this.env.valid(this.x,this.y-1) && !this.env.get(this.x,this.y-1)) { this._moveTo(this.x,this.y-1); }
...
Dude.prototype.down = function() {
	...
	if(this.env.valid(this.x+dx,this.y+1) && !this.env.get(this.x+dx,this.y+1)) {
	    this.carry._moveTo(this.x+dx,this.y+1); this.carry._fall();
	    this.carry = null;
	}
    } else {
	if(this.env.get(this.x+dx,this.y) && !this.env.get(this.x+dx,this.y+1) && !this.env.get(this.x,this.y+1)) {
	    ...
	    if(obj.type == 'block') {
		this.carry = obj;
		this.carry._moveTo(this.x,this.y+1); this.carry._fall();
	    }
	}
    }
    this._redraw();
...
Dude.prototype._walkTo = function(x,y) {
	...
	if(!this.env.get(x,y)) {
	    this._moveTo(x,y); this._fall();
	    if(this.carry) {
		if(this.env.valid(x,y+1) && !this.env.get(x,y+1)) {
		    this.carry._moveTo(x,y+1); this.carry._fall();
		} else {
		    this.carry._fall(); this.carry = null;
		}
	    }
	} else {
...
document.ongestureend = function(e) { e.preventDefault(); };
